...
Tracks multiple return dates (12/22 and 12/23) to compare which is cheaper.
"""
...
RETURN_DATES = ["2026-12-22", "2026-12-23"]
FROM_AIRPORT = "TPE"
TO_AIRPORT = "NRT"
SEAT = "economy"
PASSENGERS = 1
...
CURRENCY = "TWD"
MAX_STOPS = 0
QUERY_DELAY_SECONDS = 10  # 兩次查詢之間的間隔，降低被判定為機器人流量的風險
...
def flight_row(flight: object, return_date: str, current_price_level: str) -> dict[str, str]:
    return {
...
def query_one_combination(return_date: str) -> list[dict[str, str]]:
    """查詢單一「去程 + 回程日期」組合，回傳符合條件的航班列。"""
    flights = [
        FlightData(
            date=OUTBOUND_DATE,
    ...
    return [
        flight_row(flight, return_date, price_level)
...
def main() -> None:
    ...
    for index, return_date in enumerate(RETURN_DATES):
        ...
        rows = query_one_combination(return_date)
        all_new_rows.extend(rows)
        print(f"{OUTBOUND_DATE} -> {return_date}: matched {len(rows)} fares")
    ...
    print(f"Recorded {len(all_new_rows)} matching fares; {len(history)} total history rows.")
